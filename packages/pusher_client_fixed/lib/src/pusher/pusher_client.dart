...
class PusherClient extends StreamHandler {
  ...
  /// Creates a [PusherClient] -- returns the existing singleton or creates new one.
  /// PATCH: Always calls _init() to ensure new auth tokens are applied.
  factory PusherClient(
    ...
    // Always call _init() — Kotlin side has been patched to always reinit
    // so new tokens are applied even when singleton already exists.
  ...
  /// PATCH: Resets the Dart singleton so the next PusherClient() call
  /// creates a fresh instance. Call this before relogin to ensure
  /// new auth headers (token) are applied correctly.
