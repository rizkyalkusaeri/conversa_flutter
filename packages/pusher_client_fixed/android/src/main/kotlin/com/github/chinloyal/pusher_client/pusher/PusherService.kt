...
class PusherService : MChannel {
    ...
    private fun init(call: MethodCall, result: Result) {
        ...
        // PATCH: Always reinitialize when called, so that new auth tokens
        // (after logout/login) are properly applied to the HttpAuthorizer.
        // The original guard `if(_pusherInstance == null)` caused stale tokens
        // to be used after relogin, resulting in auth 500 errors.
    ...
    private fun disconnect(result: Result) {
        ...
        // PATCH: Null out the instance so the next init() call creates a fresh
        // Pusher with updated auth headers (new token after relogin).
